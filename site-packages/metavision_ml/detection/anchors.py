...
class Anchors(nn.Module):
    ...
    def __init__(self,  num_levels=4,
                 base_size=32,#原本为32. 到时候针对小目标，可以考虑改成8或者16
        ...
        self.anchor_list = getattr(sys.modules[__name__], anchor_list) #PSEE_ANCHORS = [(1. / 3, 1), (0.5, 1), (1, 1), (1, 1.5), (2, 1), (3, 1)]
        self.fg_iou_threshold = fg_iou_threshold
        self.bg_iou_threshold = bg_iou_threshold
        self.num_anchors = len(self.anchor_list) # 6
        self.allow_low_quality_matches = allow_low_quality_matches
        self.max_decode = max_decode
        self.variances = variances
        self.anchor_generators = nn.ModuleList()
        for i, box_size in enumerate(self.sizes): #[2^5, 2^6, ... , 2^9]
    ...
    def encode(self, features, x, targets):
        ...
        """
        anchors = self(features, x) #forward的过程中会得出self.anchors和self.anchors_xyxy
    ...
    def encode_anchors(self, anchors, anchors_xyxy, targets):
        ...
        Returns:
            ...
            cls (torch.Tensor): encoded anchors classification targets
        """
        gt_boxes, gt_labels, sizes = self._pad_boxes(targets, anchors.device)#把bbox的数量统一调整为最多的那个，不够的会进行padding.
        # gt_boxes 张量: [len(targests), max_labels, 4] ([x1,y1,x2,y2]) 填充部分的值为-2
        # gt_labels 张量: [len(targests), max_labels] (cls_id) 填充的值为-2
        # sizes, 列表,长度为len(targets) 表示每个有效delta_t的gt_bbox数量
        ...
        boxes = torch.gather(gt_boxes, 1, index) #[N,A,4] 代表每个prior所对应的那个gt框xyxy
        loc_targets = box.bbox_to_deltas(boxes, anchors[None], self.variances) #[N,A,4](delta_x, delta_y, scale_W, scale_h) 整体的逻辑就是 destination ÷ source . 得到每个prior应有的偏移量,已经经过了encode.

        labels = torch.gather(gt_labels, 1, best_target_per_prior_indices) #[N,A] 代表每个prior所对应的那个gt框id
        labels[mask_ign] = -1
        labels[mask_bg] = 0
        cls_targets = labels.view(-1, len(anchors)) #[N,A] 每个prior对应的id
    ...
    def decode(
        ...
        """
        anchors = self(features, x) #[A,4](xywh)

        # loc_preds [N, C] (do not include background column)
        box_preds = box.deltas_to_bbox(loc_preds, anchors, self.variances) #[N,A,4](xyxy)

        # Decoding
        decoded_dict = self.batched_decode(box_preds, scores, score_thresh, nms_thresh, topk=max_boxes_per_input)
        # decoded_dict: {"boxes": boxes[keep], "scores": scores_out[keep], "labels": labels, "batch_index": batch_index}
        # shape 分别是 [num_valid_final,4](xyxy) / [num_valid_final,]预测概率(0到1) / [num_valid_final,] 预测类别(1到7) / [num_valid_final,] 每个框来自于N中的哪一个 0到N-1

        num_tbins = len(scores) // batch_size #有对应label的ev_repr(frame)实际能凑满多少个tbin
    ...
    def forward(self, xs: List[torch.Tensor], x: torch.Tensor):
        ...
        shapes = torch.tensor([item.size()[-2:] for item in xs]).int() #tensor, 形状是[5,2] 代表每个level的hw
        imsize = x.size()[-2:] #最开始进入网骆的hw
        strides = torch.tensor([int(imsize[-1] // shape[-1]) for shape in shapes]) #tensor([8,16,32,64,128])

        device = self.anchors.device #默认在cpu上
    ...
    def _forward(self, shapes: torch.Tensor, strides: torch.Tensor):
        ...
        if not torch.eq(shapes, self.last_shapes).any().item():
            ...
            for i, anchor_layer in enumerate(self.anchor_generators):
                shape = shapes[i]
                stride = strides[i]
                anchors = anchor_layer(shape, stride) #得到当前分辨率下的prior框,[height*width*num_anchors, 4] 第一维指当前分辨率下的anchor总数,第二维的4代表每个anchor的[xc,yc,w,h](在原始分辨率下)
                default_boxes.append(anchors)
            self.anchors = torch.cat(default_boxes, dim=0) #5个level下的所有default_anchors [total,4]
            self.anchors_xyxy = box.xywh2xyxy(self.anchors) #第二维的4转变为xyxy形式
    ...
    def _pad_boxes(self, targets, device):
        ...
        # gt_padded [len(targests), max_labels, 5] ([x1,y1,x2,y2,cls_id]) 填充部分的值为-2 ;  len(targests) 即为 N
        # num_boxes list,长度为len(targets) 表示每个有效delta_t的gt_bbox数量
    ...
    def _iou_assignement(self, gt_boxes, gt_labels, anchors_xyxy, sizes):
        ...
        """
        ious = box.batch_box_iou(anchors_xyxy, gt_boxes)  # [N, A, M] / [len(valid), total, max_size]

        # make sure to not select the dummies
        mask = (gt_labels == -2).float().unsqueeze(1)
        ious = (-1 * mask) + ious * (1 - mask) #对于padding出来的gt框，对应的所有iou都是-1; 对于正常gt框没影响。
    ...
    def _expand_decoded_boxes(self, decoded_dict, batch_size, num_tbins):
        ...
        bidx, sidx = decoded_dict.pop('batch_index').sort() #来自N中的哪一个按照升序排列，前面的是升序排列后的结果，后面是对应的排列索引 相对于[num_valid_final,]
        bidx_vals, sizes = torch.unique(bidx.long(), return_counts=True) #第一个是去重后出现的bidx(即N中的哪些个,升序) 第二个是每个bidx的出现次数
        sidx_list = sidx.split(sizes.cpu().numpy().tolist()) #元组,里面有若干个子张量(一维),把每个样本对应的索引进行了聚类分组 这里的索引都是相对于[num_valid_final,]
    ...
    def batched_decode(self, boxes, scores, score_thresh, nms_thresh,
        ...
        # 默认配置下， scores 不会有变化 [N,A,7] 代表每个anchor对每一实际类的预测概率
        # idxs: [N, A, 7]. 核心是[N, 7],比如batch中第一个样本对应的是[0,1,2,3,4,5,6]/第二个样本对应的是[7,8,9,10,11,12,13]/... 然后对于每个prior_anchor都共享。

        if not self.max_decode:
            boxes = boxes.unsqueeze(2).expand(-1, num_anchors, num_classes, 4).contiguous() #[N,A,4]->[N,A,1,4]->[N,A,7,4]

        # filtering boxes by score
        scores_out, score_indices = self._score_filtering_per_image(scores, score_thresh, topk) #首先对每个frame取最高的前topk个score: [N, topk] 然后展平并通过score_thresh筛选
        # scores_out [num_valid,] 代表[N,topk]个分数中符合score_thresh的分数值，从[N*topk,]中筛选得到
        # score_indices [num_valid,] 代表每个valid分数的对应idx:是在N*A*7展平后的全局索引

        boxes = boxes.view(-1, 4)[score_indices].contiguous() #[num_valid,4]代表经过topk和score_thresh筛选后的有效bbox (xyxy)
        idxs = idxs.view(-1)[score_indices].contiguous() #[num_valid,]
        # apply NMS to boxes with sufficient score
        keep = batched_nms(boxes, scores_out, idxs, nms_thresh) #会隐式地根据idxs(类别)对boxes进行分组，各组内做独立的 NMS,不同组之间互不影响

        idxs = idxs[keep] #[num_valid_final,]
        labels = idxs % num_classes + 1 #每个最终合法框对应的类别 1到7
        batch_index = idxs // num_classes #代表每个最终合法框来自于batch中第几个样本
    ...
    def _score_filtering_per_image(self, scores, score_threshold, topk_per_image):
        ...
        Returns:
            ...
            idxs (torch.Tensor): indices of best scores
        """
        batch_size, num_anchors, num_classes = scores.shape #[N,A,7]
        topk_per_image = min(topk_per_image, num_anchors * num_classes)
        scores = scores.reshape(batch_size, num_anchors * num_classes) #第二维度肯定是某个框对某个实际类别的预测分数 [N,7*A]
        scores_filtered, scores_filtered_indices = torch.topk(scores, topk_per_image, dim=1)
        #scores_filtered [N,topk] 每一行里都是降序的score / scores_filtered_indices [N, topk] 每个idx都是[0, num_anchors*num_classes-1] 代表在第二维度的展平序号

        scores_filtered = scores_filtered.view(-1)

        offset = torch.arange(0, scores.numel(), num_anchors * num_classes)[:, None] #[N,1]
        scores_filtered_indices += offset.to(scores_filtered_indices) #[N, topk],但是每个idx都加上了对应batch的offset,代表在N*A*7里面的全局序号
        scores_filtered_indices = scores_filtered_indices.view(-1) #[N*topk,] 但是每个idx都代表在N*A*7里面的全局序号
