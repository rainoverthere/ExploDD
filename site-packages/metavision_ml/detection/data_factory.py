...
def psee_data(hparams: argparse.Namespace, mode: str, empty_label: str = 'empty') -> SequentialDataLoader:
    ...
    array_dim = (hparams.num_tbins, hparams.in_channels, hparams.height, hparams.width) #原本的(全累计)
    # array_dim = (hparams.num_tbins, 2*hparams.in_channels, hparams.height, hparams.width) #改为了往前积累1个delta_t，所以通道数翻倍
    ...
    class_lookup = box_api.create_class_lookup(
        label_map_path, hparams.classes) #这里的lookup table是一个列表，长度和json文件的总类别数一致，不想要的类别都默认为-1,想要的类别从左到右依次为1,2,3...
