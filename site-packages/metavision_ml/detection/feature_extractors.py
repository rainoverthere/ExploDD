...
from metavision_core_ml.core.modules import ConvLayer, PreActBlock, FEBlock
...
class Vanilla(nn.Module):
    ...
    def __init__(self, cin=1, base=16, cout=256):
        ...
        self.conv1 = SequenceWise(nn.Sequential(
            ConvLayer(cin, self.base * 2, kernel_size=7, stride=2, padding=3, norm='BatchNorm2d'), #cin->2*base ; 高宽减半 [tn, 2*base, h/2, w/2]
            PreActBlock(self.base * 2, self.base * 4, 2), #[tn, 4*base, h/4, w/4]
            PreActBlock(self.base * 4, self.base * 4, 1), #[tn, 4*base, h/4, w/4] #SE+残差连接的思想
            PreActBlock(self.base * 4, self.base * 8, 1), #[tn, 8*base, h/4, w/4]
        ))

        # # 我的
        # self.conv1 = SequenceWise(nn.Sequential(
        #     ConvLayer(cin, self.base * 2, kernel_size=7, stride=2, padding=3, norm='BatchNorm2d'), #cin->2*base ; 高宽减半 [tn, 2*base, h/2, w/2]
        #     FEBlock(self.base * 2, self.base * 4, 2), #[tn, 4*base, h/4, w/4]
        #     FEBlock(self.base * 4, self.base * 4, 1), #[tn, 4*base, h/4, w/4]
        #     FEBlock(self.base * 4, self.base * 8, 1), #[tn, 8*base, h/4, w/4]
        # ))
    ...
    def forward(self, x):
        x = self.conv1(x) #[t, n, cin, h, w]->[t, n, 8*base, h/4, w/4]
        outs = []
        for conv in self.conv2:
            x = conv(x) #这里输出的x其实是lstm的h
            y = time_to_batch(x)[0]
            outs.append(y)
        return outs
    #outs包含5个level的特征: [tn,cout,h/8,w/8] [tn,cout,h/16,w/16] [tn,cout,h/32,w/32] [tn,cout,h/64,w/64] [tn,cout,h/128,w/128]
