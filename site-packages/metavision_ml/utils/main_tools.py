...
def check_input_power_2(event_input_height, event_input_width, height=None, width=None):
    ...
    def _check(event_input_dim, dimension, dim_name):
        if height is not None:
            ...
            n = int(quotient)
            assert (n & (n - 1) == 0) and n != 0, msg #没有问题,这里是经典算法，判断n必须是2的幂次方. &是二进制的与运算
...
def infer_preprocessing(params, h5path=None):
    ...
    with h5py.File(h5path, "r") as f:
        dset = f['data']
        mode = f["data"].attrs.get("mode", "delta_t") #'delta_t'
        n_events = f["data"].attrs.get("n_events", 0) # 0
        delta_t = dset.attrs["delta_t"] #预处理时的delta_t=50000
        preprocess_name = dset.attrs["events_to_tensor"].decode("utf-8") #'event_cube_paper'
        preprocess_kwargs_keys = get_preprocess_kwargs_keys(preprocess_name) #['preprocess_dtype','split_polarity','max_incr_per_pixe'](event_cube_paper)
        preprocess_kwargs = {}
        for preprocess_kwargs_key in preprocess_kwargs_keys:
            if preprocess_kwargs_key == "preprocess_dtype":
                continue
            #==================================================自己添加的
            if preprocess_kwargs_key == "max_incr_per_pixel":
                preprocess_kwargs[preprocess_kwargs_key]=None #和自己的预处理保持一致
                continue
            #==================================================自己添加的
        ...
        channels = dset.shape[1] # 10 (event_cube_paper)
