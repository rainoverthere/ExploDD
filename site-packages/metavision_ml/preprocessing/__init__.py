...
class CDProcessor(object):
    ...
    def __init__(self, height, width, num_tbins=5, preprocessing='histo', downsampling_factor=0,
        ...
        self.dtype = self.preprocess_kwargs.pop(
            'preprocess_dtype') if "preprocess_dtype" in self.preprocess_kwargs else np.uint8 #np.dtype('float32')

        if downsampling_factor>0 and "quantized" in preprocessing: #不会执行
            #we need float 32 when we downsample quantized tensors
            self.dtype = np.dtype('float32')
        in_channels = self.preprocess_dict['cin']
        self.array = np.zeros((num_tbins, in_channels, height, width), self.dtype)#np.dtype('float32')
