...
def viz_event_cube_rgb(im, split_polarity=True):
    ...
    if split_polarity:
        ...
        '''
        原本是这三行代码：
        im = im[:3] #这里的可视化可以考虑把img沿着bin维度求和，得到[H, W], 然后再调用 viz_diff 得到一张灰度图
        # im = im.reshape(3, 1, h, w) #这一行不需要
        rgb = np.concatenate([viz_diff(im[i])[..., None] for i in range(3)], axis=2)
        '''
        summed = np.sum(im, axis=0)

        # 第二步：构建 [h, w, 3] 的 RGB 图像
        h, w = summed.shape

        # # 设置颜色 这种方法可能会出现偶尔的爆闪(比如mini_dataset的train split)
        # rgb = np.zeros((h, w, 3), dtype=np.uint8)
        # rgb[summed > 0] = [255, 255, 255]      # 白色
        # rgb[summed < 0] = [0, 0, 0]            # 黑色
        # rgb[summed == 0] = [127, 127, 127]     # 灰色

        # 设置颜色
        # rgb = np.full((h, w, 3), 127, dtype=np.uint8) #默认为灰色
        rgb = np.full((h, w, 3), 242, dtype=np.uint8) #默认为白灰色
        rgb[summed > 0] = [255, 0, 0]      # rgb红色
        rgb[summed < 0] = [0, 0, 255]            # rgb蓝色
