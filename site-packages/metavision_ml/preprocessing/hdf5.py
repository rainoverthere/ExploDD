...
from metavision_core.event_io import EventsIterator, EventNpyReader, NPYEventsIterator
...
def write_to_hdf5(path, start_ts=0, output_folder=".", delta_t=50000,
    ...
    """
    num_tbins = 1
    dataset_size_increment = 100  # dataset size is increased by this much until we know the actual size (h5文件的初始化，不用修改)
...
def write_to_hdf5_npy(path, start_ts=0, output_folder=".", delta_t=100,
                  preprocess="histo", height=None, width=None, max_duration=None,
                  store_as_uint8=False, mode="delta_t", n_events=0, preprocess_kwargs={}):
    """
    Generates a single HDF5 file of frames at a regular interval for dataset caching.
    ...
    dataset_size_increment = 100  # dataset size is increased by this much until we know the actual size (h5文件的初始化，不用修改)
    total_tbins_delta_t = num_tbins * delta_t  # this should be 0 if mode = "n_events"
    # if mode == "n_events": #暂时只考虑delta_t模式
    #     assert delta_t == 0, "For mode = n_events, delta_t should be 0!"

    extension = ".h5" if max_duration is None else "_{:d}.h5"
    basename = os.path.basename(path).split('.')[0]
    # if basename.endswith("_td") or basename.endswith("_cd"):
    #     basename = basename[:-3] #应该不可能是这个
    hfilename = basename + extension
    ...
    if os.path.exists(hfilename):
        print(f"Own warning: {hfilename} already exists, skip further processing!")
        return

    NPY_iterator = NPYEventsIterator(path=path, start_ts=start_ts, delta_t=total_tbins_delta_t, relative_timestamps=True)

    event_input_height, event_input_width = 720, 1280 #npy模拟事件暂时只考虑原始分辨率直接输入
    height = height if height is not None else event_input_height #720
    width = width if width is not None else event_input_width #1280

    cd_proc = CDProcessor(height, width, num_tbins=num_tbins, preprocessing=preprocess,
                          downsampling_factor=int(math.log2(event_input_height // height)),
                          preprocess_kwargs=preprocess_kwargs)
    '''
    preprocess_kwargs : {'split_polarity': False, 'max_incr_per_pixel': None}
    '''

    array_dim = cd_proc.get_array_dim() #[1,10,720,1280]
    preprocess_dtype = cd_proc.get_preprocess_dtype() #np.dtype('float32')
    last_ts = 0
    last_n_ev = 0
    ...
    attrs = {"events_to_tensor": np.string_(preprocess), "delta_t": np.uint32(delta_t),
             "event_input_width": event_input_width, "event_input_height": event_input_height, "shape": array_dim[1:],
             "mode": mode, "n_events": n_events}
    attrs.update(preprocess_kwargs) # {'split_polarity': False, 'max_incr_per_pixel': None}

    n_file = 0
    print(f"Processing {os.path.basename(path)}, storing results in {hfilename}")
    h5w = HDF5Writer(hfilename if max_duration is None else hfilename.format(n_file), "data", array_dim[1:],#[10,720,1280]
                     dtype=preprocess_dtype, attrs=attrs, store_as_uint8=store_as_uint8)
    h5w.dataset_size_increment = dataset_size_increment

    for index, events in tqdm(enumerate(NPY_iterator), position=1):

        if mode == "delta_t" and max_duration is not None and index * total_tbins_delta_t - n_file * max_duration >= max_duration: #默认情况下不会执行
            # the current file is finished, if data has been produced, we close it
            if h5w.index:
                # finally we reshape the dataset to its exact value
                last_ts = last_ts % max_duration
                ...
                h5w = HDF5Writer(hfilename.format(n_file), "data",
                                 ...
                                 store_as_uint8=store_as_uint8)
                h5w.dataset_size_increment = dataset_size_increment
        elif mode == "n_events" and max_duration is not None: #默认不会执行
            # we check if we will exceed max_duration
            if len(events) and last_ts + events['t'][-1] >= max_duration:
                if last_n_ev < n_events:
                    h5w.set_cursor(h5w.index - 1)  # drop the last frame since we don't have enough events
        ...
        proc_dt = total_tbins_delta_t if mode == "delta_t" else None #delta_t
        array = cd_proc(events, delta_t=proc_dt)
...
def generate_hdf5(paths, output_folder, preprocess, delta_t, height=None, width=None,
    ...
    #===========================================================================================
    event_input_height = 720
    event_input_width = 1280
    if paths[0].endswith(".npy"):
        print('The input files are *.npy !')#处理模拟.npy数据的时候会有这么一行字
    if paths[0].endswith('.raw'):
        print('The input files are *.raw !') #处理真实数据的时候是会有这么一行字的
        event_input_height, event_input_width = EventsIterator(paths[0]).get_size() #原本只有这一行
    # ===========================================================================================

    for i, path in enumerate(paths):
        assert path.split('.')[-1].lower() in ('dat', 'raw', 'npy'), f"input file nb{i}: {path} must be .raw or .dat or .npy"
    ...
    check_input_power_2(event_input_height, event_input_width, height=height, width=width) #如果没有主动指定需要处理成的h/w，那么这一步相当于没用

    starts_ts = _check_start_ts(start_ts, paths)
    # HANDLE BOX LABELS
    if box_labels: #默认情况下不会执行
    ...
    args_list = [{"path": path, "start_ts": start_ts, "output_folder": output_folder, "delta_t": delta_t,
                  ...
                  "preprocess_kwargs": preprocess_kwargs.copy()} #到这里，height和width都仍然是None(如果不进行下采样的话)
