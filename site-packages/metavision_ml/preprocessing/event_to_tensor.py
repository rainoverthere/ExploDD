...
def _event_cube(xypt, output_array, total_tbins_delta_t, downsampling_factor=0, split_polarity=True,
                max_incr_per_pixel=5): #这是以delta_t为单位来逐个处理的
    """
    ...
    if split_polarity:
        ...
        tensor = output_array.reshape(-1, 1, height, width)#执行这个

    t0 = 0 #由于EventIterator的relative_ts=True 这里的t0设为0
    nbins = tensor.shape[0] #10

    k = 1. / (max_incr_per_pixel * 4 ** downsampling_factor) if max_incr_per_pixel is not None else 1 #k=1
    ...
    for i in range(xypt.shape[0]):
        ...
        if 0 <= lbin < nbins:
            ...
            # tensor[lbin, p, y, x] = max(-1, min(new_value, 1)) #这里可以直接把new_value传过去，不用约束上下限，因为爆炸等场景肯定是需要通过数值大小把亮度变化体现出来的
            tensor[lbin, p, y, x] = new_value
        if rbin < nbins:
            new_value = tensor[rbin, p, y, x] + right_value * k
            # tensor[rbin, p, y, x] = max(-1, min(new_value, 1)) #这里也同理。
            tensor[lbin, p, y, x] = new_value
