...
from .transformations import transform_sequence, seq_h_flip, labels_h_flip
...
def _downsample_clip(frames, output_array, max_val=None):
    ...
    if h_src != h_dst or w_src != w_dst:
        ...
        #real_tbin=frames.shape[0]
        #output_array[:real_tbin]=frames
...
class SequentialDataset(data.Dataset):
    def __init__(self, files, delta_t, preprocess_function_name, array_dim,
        ...
        self.scheduler = Scheduler.create_schedule(files, durations, self.delta_t, self.num_tbins, batch_size,
                                                   max_consecutive_batch=None, shuffle=False, padding=padding)#是一个 Scheduler 实例，且拥有 cumulative_time_table_per_batch 等属性
    ...
    def _load_sequences(self, metadata, batch_start_time, duration):
        ...
        if file_extension in (".dat",):
            ...
            precomputed_frames = self._load_hdf5(metadata, batch_start_time, duration)#取出这个item在原本h5文件里对应部分的tensor, [num_tbin,c0,h0,w0]
    ...
    # def _load_hdf5(self, metadata, batch_start_time, duration):
    #     """Fetches precomputed tensors and constructs cumulative event voxel representation."""
    #     with h5py.File(metadata.path, "r") as f:
    #         assert f["data"].attrs.get("mode", "delta_t") == "delta_t", "only delta_t mode supported"
    #         delta_t = f["data"].attrs["delta_t"]
    #         assert delta_t > 0

    #         start_idx = batch_start_time // delta_t
    #         end_idx = (batch_start_time + duration) // delta_t
    #         num_tbins = end_idx - start_idx

    #         # Always read one more frame to the front (if available) for accumulation
    #         read_start_idx = max(start_idx - 1, 0)
    #         array = f['data'][read_start_idx:end_idx]  # shape: [num_tbins + 1 or num_tbins, c, h, w]

    #         c, h, w = array.shape[1:]
    #         cumulative_array = np.zeros((num_tbins, 2 * c, h, w), dtype=array.dtype)

    #         for i in range(num_tbins):
    #             current = array[i + (1 if start_idx > 0 else 0)]
    #             if i == 0 and start_idx == 0:
    #                 prev = np.zeros_like(current)  # no previous frame exists
    #             else:
    #                 prev = array[i + (0 if start_idx > 0 else -1)]
    #             cumulative_array[i] = np.concatenate([prev, current], axis=0)
    ...
    def __getitem__(self, index):
        ...
        """
        metadata, batch_start_time, duration, data_augmentation = self.scheduler[index]
        #这里调用的是scheduler的__getitem__函数
        # metadata 是一个 FileMetadata 对象，包含当前这个item(tbin*delta_t)所属的序列的全部信息。
        # batch_start_time 代表当前这个item (tbin*delta_t) 的开始时间(是一个相对时间，是相对于其所属那个场景的起始时间的)
        # duration 当前这个item的duration (数值上的tbin*delta_t)
        # 如果这个item是padding出来的，那么会得到 空的FileMetadata, 0, 0
        if metadata.is_padding():
            ...
            empty_event_bbox = np.array([], dtype=EventBbox)
            return (seq, [empty_event_bbox for _ in range(self.num_tbins)],
                    keep_memory, frame_is_labeled, (metadata, batch_start_time, duration))

        seq = self._load_sequences(metadata, batch_start_time, duration) #[num_tbin,c,h,w]
        #这个item对应的ev数组(已经经过了缩放,从预处理h5文件中的hw再次缩放至新指定的更小的hw,但要注意，新指定的hw仍然必须为最原始分辨率下采样2^n倍。如果没有最新指定，则默认会和预处理h5保持一致)

        # keep_memory is false (i.e. we must reset) only at the beginning of a new
        # sequence : when batch_start_time == metadata.start_ts
        keep_memory = float(batch_start_time > metadata.start_ts) #当取到某个场景的第一个item(num_tbin*delta_t)时候，keep_memory = false = 0.
        seq = torch.from_numpy(seq)

        # data aug
        seq = transform_sequence(seq, metadata, self.transforms, base_seed=self.scheduler.base_seed)#viz_data的transform=None

        labels, frame_is_labeled = self.load_labels(metadata, batch_start_time, duration, seq)#最新的hw形状会从seq中读取，然后用于缩放对应的bboxes，下面得到的labels都已经经过了最新缩放以及class_id调整等。
        #labels是一个长度为num_tbin的列表，每个元素都是那个delta_t里面包含的bboxes(结构化数组)
        #frame_is_labeled是一个长度为num_tbin的布尔数组，表明对应的那个delta_t的tensor是否有至少一个bbox.

        video_infos = (metadata, batch_start_time, duration)

        #======================================================================
        #在训练阶段, 对seq和labels进行数据增强
        if data_augmentation == 'no_aug':
            pass
        if data_augmentation == 'h_flip':
            seq=seq_h_flip(seq)
            labels=labels_h_flip(labels)
        #======================================================================
...
def collate_fn(data_list):
    ...
    tensors = torch.stack(tensors, 1) #[num_tbin, batch_size, c, h, w]

    # if some of the file metadata have a valid labelling_delta_t and some don't, this stack operation will fail
    frame_is_labeled = np.stack(frame_is_labeled, 1) #[num_tbin, batch_size]
    frame_is_labeled = torch.from_numpy(frame_is_labeled)
    mask_keep_memory = torch.FloatTensor(mask_keep_memory).view(-1, 1, 1, 1) #[4,1,1,1]

    t, n = tensors.shape[:2]

    tn_labels = [[labels[i][t] for i in range(n)] for t in range(t)] #每个delta_t对应的labels
...
class SequentialDataLoader(object):
    ...
    def __init__(self, files, delta_t, preprocess_function_name, array_dim,
        ...
        self.device = torch.device('cpu')#虽然这里是cpu，但是在主训练代码train_detection.py中最后会显式指定，让pl使用单GPU进行训练
    ...
    def __iter__(self):
        ...
        self.dataloader = iter(data.DataLoader(self.dataset, batch_size=max(1, self.dataset.batch_size),
                                               ...
                                               collate_fn=collate_fn, pin_memory=False)) #把DataLoader对象变为迭代器，从而可以调用__next__函数
        self.step = 0
        return self
        ...
        for ind, data_dic in enumerate(self):
            ...
            for t in range(batch.shape[0]):
                ...
                for index, im in enumerate(batch[t]):
                    ...
                    img = self.get_vis_func()(im, split_polarity=False)#注意需要显式指定 split_polarity=False(默认为True)/ 同时，这个可视化函数可能并不合理，需要自己改写一下。
                    ...
                    if metadata[index][0].padding:
                        name='Padding Sequence'
                    else:
                        name = metadata[index][0].path.split('/')[-1] #basename
