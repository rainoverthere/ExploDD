...
def _rounding(duration, total_tbins_delta_t, max_batch=None, rounding_tolerance=1):#rounding_tolerance原本为1000
    ...
    if isinstance(excess_duration, np.ndarray):
        ...
        #假设 duration / total_tbins_delta_t = N 点几
        #如果 excess_duration 足够大，足够接近total_tbins_delta_t (差距小于 rounding_tolerance )
        #那么最终的 rounded_duration 会等于(N+1)*total_tbins_delta_t ; 否则会截断, rounded_duration 会等于 N*total_tbins_delta_t
...
class Scheduler(object):
    ...
    def __init__(self, filesmetadata, total_tbins_delta_t, batch_size,
        ...
        #==============================================================
        #My data augmentaion
        self.data_augmentation=['no_aug' for _ in range(len(self.files))] #data_loader刚生成的时候会执行这个初始化，对于val和test都是没有任何数据增强的。但是对于训练会在shuffle函数中引入增强。
        #==============================================================
    ...
    def create_schedule(cls, files, durations, delta_t, num_tbins, batch_size,
        ...
        if len(durations) != len(files):
            ...
            durations = _get_durations(files) #列表，依次存储了所有场景的duration (us)
    ...
    def shuffle(self, seed=None):
        ...
        #====================================================================
        self.data_augmentation=self.data_aug_generation(['h_flip', 'no_aug'])
        # self.data_augmentation=self.data_aug_generation(['no_aug'])
        #====================================================================
        self.base_seed = random.randint(0, int(1e9))
        self._construct_schedule()
    
    #=======================================================================
    def data_aug_generation(self, aug_types):
        """
        aug_types: 列表，选自 'no_aug', 'h_flip', 'v_flip', 'zoom_in', 'zoom_out'
        返回: 一个长度为 len(self.files) 的列表，每个位置的增强形式随机取自 aug_types
        """
        # 生成一个长度为 len(self.files) 的列表，每个元素随机取自 aug_types
        data_augmentation = [random.choice(aug_types) for _ in range(len(self.files))]
    ...
    def __getitem__(self, index):
        ...
        if iteration_time > self.cumulative_time_table_per_batch[batch_index][-1]:
            assert self.padding
            return FileMetadata(None, None, None, None, padding=True), 0, 0, 'no_aug'
        # find which file correspond to this date in the right column of the time_table
        rel_file_index = np.searchsorted(self.cumulative_time_table_per_batch[batch_index], iteration_time)#当前这一个tbin属于对应batch容器的第几个场景
        ...
        batch_start_time = int(math.ceil((iteration_time - self.total_tbins_delta_t - time_elapsed))) #当前这一批tbin相对于当前所处场景的开始时间
        ...
        return self.files[file_index], self.files[file_index].start_ts + batch_start_time, curfile_duration, self.data_augmentation[file_index]
