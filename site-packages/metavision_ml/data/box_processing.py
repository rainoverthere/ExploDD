...
def load_boxes(metadata, batch_start_time, duration, tensor, **kwargs):
    ...
    box_events = load_box_events(metadata, batch_start_time, duration) #取出所有 大于等于batch_start_time & 小于batch_start_time+duration 的bboxes (统一改成了左开右闭)
    ...
    tmp = split_boxes(box_events, batch_start_time=batch_start_time,
                      ...
                      num_tbins=num_tbins) #对于viz_data.py: 得到的是一个item(tbin*delta_t)每个delta_t对应的bbox(结构化数组,时间上每个 delta_t 左开右闭)
    ...
    box_events = clip_boxes(box_events, width_orig, height_orig) #确保bbox在合法范围内，不超出边界，且w和h都得大于2pix(这个约束感觉没啥用，保留着应该没影响)

    # filter classes
    idx_to_filter = np.array([])
    area_box_filter = -1 #得改成负数，即不针对面积进行过滤(因为爆炸的起始面积可能很小)
    ...
    if area_box_filter > 0:
        ...
        if filter_beginning:
            time_to_filter = int(5e5) #对于每个场景，只滤前0.5s
    ...
    box_events = filter_boxes(box_events, class_lookup, idx_to_filter, ignore_filtered) #过滤不合法框，并且调整class_id(只取想要的类别，且第0类是背景)
    ...
    for i, box in enumerate(boxes):
        box = rescale_boxes(box, width_orig, height_orig, width_dst, height_dst) #缩放至需要的尺度
        ...
        # idx_to_keep *= box["w"] <= 0.8 * width_dst #对于bbox的宽度应该用不着这个约束
...
def load_box_events(metadata, batch_start_time, duration):
    ...
    """
    out = np.zeros((len(box_events),), dtype=EventBbox) #具体的EventBbox源码只有在买了EVK4之后才能看到。
...
def clip_boxes(box_events, width_orig, height_orig):
    ...
    return box_events[(box_events["w"] > 2) * (box_events['h'] > 2)] # w 和 h 都得大于2
...
def filter_boxes(box_events, class_lookup, idx_to_filter, ignore_filtered):
    ...
    box_events['class_id'] = classes[to_keep] #这里左边得到的的class_id已经是1.只考虑了想要的类别 2.过滤了不合法框(比如area=0) 3.已经默认class_id=0为背景
    return box_events
...
def split_boxes(box_events, batch_start_time, delta_t=None, num_tbins=None):
    ...
    masks = [np.where((box_events[time_field] > t) * (box_events[time_field] <= delta_t + t))[0] #np.where()返回的是一个单元素tuple，所含元素即为索引数组。所以要跟一个[0]
             for t in range(batch_start_time, batch_start_time + delta_t * num_tbins, delta_t)]

    masks = [mask[nms(box_events[mask], box_events[mask]['t'], iou_thresh=0.5)] for mask in masks] #对于viz_data，这里的nms没用,最终就是简单的把mask依次放在了一起
...
def filter_empty_tensor(
    ...
    for i, (ts, xmin, xmax, ymin, ymax) in enumerate(zip(ts_a, xmin_a, xmax_a,
        ...
        box_slice = np.abs(array[min_t:tmax, :, ymin:ymax, xmin:xmax]) #如果针对于爆炸，这个过滤步骤应该用不着，因为可能明显的爆炸区域占的面积并不大，肯定不能这么过滤。
