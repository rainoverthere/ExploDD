...
def seq_h_flip(seq):
    ...
    对seq进行水平翻转
    参数:
        seq: [num_tbin, c, h, w]的tensor
    返回:
        水平翻转后的seq
    """
    return torch.flip(seq, dims=[-1])  # 沿宽度维度(最后一个维度)翻转

def labels_h_flip(labels, image_width=1280):
    """
    对labels进行水平翻转
    参数:
        labels: 长度为num_tbin的列表，每个元素是结构化数组，包含'x','y','w','h'字段
        image_width: 图像的宽度，用于计算翻转后的x坐标
    返回:
        水平翻转后的labels
    """
    flipped_labels = []
    for label in labels:
        ...
        # 计算翻转后的x坐标: image_width - original_x - width
        flipped_label['x'] = image_width - label['x'] - label['w']
