...
class CocoEvaluator(object):
    ...
    def __init__(self, classes=("car", "pedestrian"), height=240, width=304, time_tol=0, eval_rate=-1,
                 verbose=False):
        # 这里的time_tol原本是40000
        # 在参考了RVT之后，一开始准备改成500(us)，这个数值似乎需要等于delta_t？但他们之前都是因为预测帧率和标注帧率不同步，比如预测帧率20Hz而标注帧率是60Hz
        # 由于我的情况是 bz的检测输出频率和标注频率都是2000fps，每一帧连着标，二者频率是相同的，所以可能不需要time_tol，即等价于frame_based evaluation？
        # 对于原理再认真看一看，思考是否需要把 time_tol 改为 0 
        # 结论：time_tol是需要改为0的 小于等于1/delta_t均可
        self._num_images = 0
...
def _flatten_boxes_list(gt_boxes_list, dt_boxes_list, time_tol=40000, eval_rate=-1):
    ...
    for gt_boxes, dt_boxes in zip(gt_boxes_list, dt_boxes_list):
        ...
        # all_ts = np.unique(np.concatenate((gt_boxes['t'], dt_boxes['t']))) #SDK462原本使用的是concat
        all_ts = np.unique(gt_boxes['t']) #但是RVT和psee发布的开源评估只使用了gt 最后证明这两者对于最后的评估指标是等价的，没有区别。
