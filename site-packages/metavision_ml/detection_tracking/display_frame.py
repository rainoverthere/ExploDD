...
def draw_box_events(frame, box_events, label_map, force_color=None, draw_score=True, thickness=1, is_gt=False,
    ...
    colors = choose_color(box_events, color_from, force_color=force_color) #得到每一个bbox对应的颜色(BGR)
    ...
    for i, (tlx, tly, brx, bry) in enumerate(zip(topleft_x, topleft_y, botright_x, botright_y)):
        ...
        text = label_map[box_events[i]["class_id"]] #这里可以自定义 直接就可以"boom"或者"explosion"
        if draw_score:
            text += " {:.2f}".format(box_events[i][confidence_field])
        if is_gt:
            cv2.putText(frame, text, (tlx, tly - 13), cv2.FONT_HERSHEY_SIMPLEX, 2, color, 6)
        else:
            # cv2.putText(frame, text, (int(tlx + 0.05 * (brx - tlx)), int(tly + 0.94 * (-tly + bry))),
            #             cv2.FONT_HERSHEY_SIMPLEX, 1., color, 2)
            cv2.putText(frame, text, (brx + 5, bry-5),
                        cv2.FONT_HERSHEY_SIMPLEX, 2, color, 6)#预测框的text部分稍粗
