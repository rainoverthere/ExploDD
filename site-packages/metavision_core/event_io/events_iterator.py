...
class NPYEventsIterator:
    def __init__(self, path, start_ts=0, delta_t=10000, relative_timestamps=True):
        """
        Args:
            path (str): path to .npy file (结构化数组，字段必须包含 't')
            start_ts (int): 起始时间戳，单位为微秒
            delta_t (int): 每一帧的时间间隔（以微秒为单位）
            relative_timestamps (bool): 是否把时间戳平移使得start_ts为0
        """
        self.events = np.load(path)
        self.delta_t = delta_t
        self.relative = relative_timestamps

        # # 排序保证时间递增（如果原数据已保证可以跳过）
        # self.events = np.sort(self.events, order='t')

        self.start_ts = start_ts

        self.end_ts = self.events['t'][-1]
        self.current_idx = 0
        self.current_window_start = self.start_ts

    def __iter__(self):
        return self

    def __next__(self):
        if self.current_window_start >= self.end_ts:
            raise StopIteration

        window_end = self.current_window_start + self.delta_t
        # 左开右闭：(start, end]
        mask = (self.events['t'] > self.current_window_start) & (self.events['t'] <= window_end)

        # 获取匹配的事件
        matched_events = self.events[mask]
        ...
        if self.relative:
            matched_events['t'] -= matched_events['t'][0]
