...
class SequenceWise(nn.Module):
    def __init__(self, module, ndims=5):
        ...
        self.ndims = ndims # 5
        self.module = module #一个nn.Sequential实例

        if hasattr(self.module, "out_channels"): #正常情况下 self.module 是一个nn.Sequential实例，没有'out_channels'属性
    ...
    def forward(self, x):
        if x.dim() == self.ndims - 1:
            ...
            x4, batch_size = time_to_batch(x) #x4: [t*n, c, h, w]; 
            y4 = self.module(x4)
            return batch_to_time(y4, batch_size)
...
class ConvLSTMCell(RNNCell):
    ...
    def __init__(self, hidden_dim, kernel_size, conv_func=nn.Conv2d, hard=False):
        super(ConvLSTMCell, self).__init__(hard) #hard默认为False, 即使用正常的torch.sigmoid()和torch.tanh()
    ...
    def forward(self, x):
        ...
        assert input_C == 4 * hidden_C #这个取决于更上层的ConvRNN的默认设定
        ...
        for t, xt in enumerate(xseq):
            ...
            c = f * self.prev_c + i * g
            h = o * self.tanh(c)
            result.append(h.unsqueeze(0)) #[1,n,hidden_dim,h_,w_]

            self.prev_h = h
            self.prev_c = c

        res = torch.cat(result, dim=0) #[t,n,hidden_dim,h_,w_]
        return res
...
class ConvRNN(nn.Module):
    ...
    def forward(self, x):
        y = self.conv_x2h(x) #[t,n,in_channels,h,w]->[t,n,4*out_channels,h/stride,w/stride]
        h = self.timepool(y) #[t,n,4*out_channels,h/stride,w/stride]->[t,n,out_channels,h/stride,w/stride]
        return h
